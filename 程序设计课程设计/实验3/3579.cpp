...
class Board {
    private:
        ...
        vector<pair<size_t, size_t>> obstacles; // # -> 墙
        vector<pair<size_t, size_t>> traps;     // X -> 陷阱，碰到就输
    ...
    public:
        ...
        bool move(int steps) {
            for (int i = 0; i < steps; i++) {
                ...
                if (board[new_head.first][new_head.second] == '#') continue;  // 墙一律跳过
                if (board[new_head.first][new_head.second] == 'X') return false;  // 碰到陷阱就输
