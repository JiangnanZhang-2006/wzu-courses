...
enum Cell : char { EMPTY = '.', TARGET = 'T', BOX_ON_TARGET = 'O', BOX = 'B', PLAYER = 'P', PLAYER_ON_TARGET = '@' };
...
private:
    ...
    void setPlayerAt(int x, int y) {
        board[x][y] = (char)(isTarget(x, y) ? PLAYER_ON_TARGET : PLAYER);
    }

    bool canPlayerStand(char c) const { return c == EMPTY || c == TARGET; }  // 玩家可站空地或目标
